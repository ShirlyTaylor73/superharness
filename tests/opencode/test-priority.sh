...
# Verifies that skills are resolved with correct priority: project > personal > Superharness
...
# 1. Create in Superharness location (lowest priority)
mkdir -p "$INSTALLED_PLUGIN_ROOT/skills/priority-test"
cat > "$INSTALLED_PLUGIN_ROOT/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superharness version of priority test skill
---
# Priority Test Skill (Superharness Version)

This is the SUPERHARNESS version of the priority test skill.

PRIORITY_MARKER_SUPERHARNESS_VERSION
EOF
...
if [ -f "$INSTALLED_PLUGIN_ROOT/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superharness version exists"
else
    echo "  [FAIL] Superharness version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides Superharness
echo ""
echo "Test 2: Testing personal > Superharness priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides Superharness)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERHARNESS_VERSION"; then
    echo "  [FAIL] Superharness version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|superharness" | head -10
fi

# Test 3: Test that project overrides both personal and Superharness
echo ""
echo "Test 3: Testing project > personal > Superharness priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Superharness version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit superharness: prefix bypasses priority
echo ""
echo "Test 4: Testing superharness: prefix forces Superharness version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load superharness:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERHARNESS_VERSION"; then
    echo "  [PASS] superharness: prefix correctly forces Superharness version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] superharness: prefix did not force Superharness version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
