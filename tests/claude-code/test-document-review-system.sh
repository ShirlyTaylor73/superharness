...
mkdir -p docs/superharness/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/superharness/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Read the spec-document-reviewer-prompt.md template in plugins/superharness/skills/brainstorming/ to understand the review format.

Then review the spec at $TEST_PROJECT/docs/superharness/specs/test-feature-design.md using the criteria from that template.
