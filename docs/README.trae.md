# Superharness — Trae 安装指南

在 [Trae](https://www.trae.ai)（字节跳动 AI IDE）中使用 Superharness 的完整指南。
...
cp -r superharness/plugins/superharness/skills/* /your/project/.trae/rules/
```
...
Superharness 的 SKILL.md 文件可以直接作为 Trae 的 rules 使用。Trae 会在初始化时加载 `.trae/rules/` 下的所有规则文件。
...
git pull
cp -r plugins/superharness/skills/* /your/project/.trae/rules/
```
