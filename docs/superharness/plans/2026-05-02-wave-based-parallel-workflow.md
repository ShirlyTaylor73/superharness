...
> **面向 AI 代理的工作者：** 必需子技能：使用 superharness:parallel-executing-plans（推荐）或 superharness:serial-executing-plans 逐任务实现此计划。步骤使用复选框（`- [ ]`）语法来跟踪进度。

**目标：** 把 superharness 的 plan→execute 工作流从纯串行升级为 wave 化并行，原生消除手动调用 dispatching-parallel-agents 的需要，同时保持单 agent（无 subagent 支持）平台的完全向后兼容。

**架构：** 三层改造。
1. **plan 格式层** — `writing-plans` 产出"加性覆盖式" plan：任务列表保持线性拓扑序（`serial-executing-plans` 直接走），每个任务头部新增 5 行 metadata（依赖/文件集/导出接口/消费接口/复杂度），文末新增 `## 并行执行图` 章节（仅 `parallel-executing-plans` 解析使用）。
2. **执行调度层** — `parallel-executing-plans` 内联 wave 调度：wave 内并发派发 implementer + spec-reviewer，wave 间阻塞 gate；commit 由 controller 在 wave 收口串行执行；修复回路并发，3 次失败逃生口，修复 subagent 不重读文件。
...
**修改：**
- `skills/serial-executing-plans/SKILL.md` — 加忽略 `## 并行执行图` 章节的说明
...
- `skills/parallel-executing-plans/SKILL.md` — 重写流程：wave 调度 + 修复回路 + final wave 四审 + 用户闸门
- `skills/parallel-executing-plans/implementer-prompt.md` — 去 commit 步骤 + 文件集约束 + 修复模式分支
- `skills/parallel-executing-plans/spec-reviewer-prompt.md` — 明确 task-local 范围

**新建：**
- `skills/parallel-executing-plans/wave-final-spec-prompt.md` — F1 全 plan 规格合规审查
- `skills/parallel-executing-plans/wave-final-quality-prompt.md` — F2 跨任务代码质量审查（吸收原 code-quality-reviewer 内核，同任务删除原文件）
- `skills/parallel-executing-plans/wave-final-manual-qa-prompt.md` — F3 真实手动 QA
- `skills/parallel-executing-plans/wave-final-scope-fidelity-prompt.md` — F4 范围保真检查

---

## 任务

### 任务 1：serial-executing-plans 加忽略 wave 章节说明

**依赖：** 无
**文件集：** `skills/serial-executing-plans/SKILL.md`
**导出/变更接口：** 无
**消费接口：** 无
**复杂度：** quick
...
`skills/serial-executing-plans/SKILL.md`：
...
```bash
git add skills/serial-executing-plans/SKILL.md
git commit -m "docs(serial-executing-plans): 让 serial-executing-plans 显式忽略 plan 中的并行执行图章节"
```
...
- new_string：`description: 非 plan 场景下，2+ 个独立任务需要并行派发时使用（典型：多个独立 bug 排查、多文件并行验证、多分支批量重试）。Plan 驱动场景请使用 parallel-executing-plans。`
...
> `parallel-executing-plans` 已经原生包含 wave 化并行调度（wave 内并发实现 + wave 间阻塞 gate + Wave FINAL 四审）。
...
**依赖：** 任务 3
**文件集：** `skills/parallel-executing-plans/SKILL.md`
...
`skills/parallel-executing-plans/SKILL.md`：
...
`skills/parallel-executing-plans/SKILL.md`：
- old_string：`**替代工作流：**\n- **superharness:serial-executing-plans** - 用于并行会话而非同会话执行`
- new_string：
```
**替代工作流：**
- **superharness:serial-executing-plans** - 用于并行会话而非同会话执行（不支持子代理的平台必备）

**通用并行派发模式（参考用）：**
- **superharness:dispatching-parallel-agents** — 本 skill 在 plan 场景下的并行调度纪律是该 skill 通用模式的具体化。单个 subagent 的 prompt 写法、范围聚焦、约束设置等通用技巧仍参考其"模式"章节。
```

- [ ] **步骤 9：Commit**

```bash
git add skills/parallel-executing-plans/SKILL.md
...
**依赖：** 任务 3
**文件集：** `skills/parallel-executing-plans/spec-reviewer-prompt.md`
**导出/变更接口：** 无
...
`skills/parallel-executing-plans/spec-reviewer-prompt.md`：
...
```bash
git add skills/parallel-executing-plans/spec-reviewer-prompt.md
...
**依赖：** 任务 5
**文件集：** `skills/parallel-executing-plans/implementer-prompt.md`
...
`skills/parallel-executing-plans/implementer-prompt.md`：
- old_string：
```
    ## 你的工作
...
```bash
git add skills/parallel-executing-plans/implementer-prompt.md
...
**依赖：** 任务 5
**文件集：** `skills/parallel-executing-plans/wave-final-spec-prompt.md`
...
新建 `skills/parallel-executing-plans/wave-final-spec-prompt.md`，内容：
...
```bash
git add skills/parallel-executing-plans/wave-final-spec-prompt.md
...
**依赖：** 任务 5
**文件集：** `skills/parallel-executing-plans/wave-final-quality-prompt.md`, `skills/parallel-executing-plans/code-quality-reviewer-prompt.md`
...
新建 `skills/parallel-executing-plans/wave-final-quality-prompt.md`，内容（吸收原 code-quality-reviewer-prompt.md 的"文件级质量检查"4 项）：
...
```bash
git rm skills/parallel-executing-plans/code-quality-reviewer-prompt.md
```

- [ ] **步骤 3：Commit**

```bash
git add skills/parallel-executing-plans/wave-final-quality-prompt.md
...
**依赖：** 任务 5
**文件集：** `skills/parallel-executing-plans/wave-final-manual-qa-prompt.md`
...
新建 `skills/parallel-executing-plans/wave-final-manual-qa-prompt.md`，内容：
...
```bash
git add skills/parallel-executing-plans/wave-final-manual-qa-prompt.md
...
**依赖：** 任务 5
**文件集：** `skills/parallel-executing-plans/wave-final-scope-fidelity-prompt.md`
...
新建 `skills/parallel-executing-plans/wave-final-scope-fidelity-prompt.md`，内容：
...
```bash
git add skills/parallel-executing-plans/wave-final-scope-fidelity-prompt.md
...
- **Wave 1**（无依赖，3 个任务并发）：
  - 任务 1：serial-executing-plans 加忽略说明
...
- **Wave FINAL**（所有任务完成后并发派发 4 个 reviewer）：
  ...
  - **F3 自举验证：** 用改造后的 `writing-plans` 写一个最小 dummy plan，用改造后的 `parallel-executing-plans` 跑通——验证新版 skill 真实可用。本会话用旧版 skill 实施，存在循环依赖，F3 须在下一次新会话中由人执行。
...
| serial-executing-plans 忽略 wave 章节 + metadata 行 | 任务 1 步骤 2 callout |
...
计划已完成并保存到 `docs/superharness/plans/2026-05-02-wave-based-parallel-workflow.md`。两种执行方式：
