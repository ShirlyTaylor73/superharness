# Superharness — Hermes Agent 安装指南

在 [Hermes Agent](https://github.com/NousResearch/hermes-agent) 中使用 Superharness 的完整指南。
...
cp -r superharness/plugins/superharness/skills /your/project/.hermes/skills
```
...
如果希望全局使用 Superharness skills，可以在 `~/.hermes/config.yaml` 中配置：

```yaml
skills:
  external_dirs:
    - /path/to/superharness/plugins/superharness/skills
```
...
上表是当前维护的 Hermes Agent 工具映射。旧版曾把扩展映射放在 `using-superpowers` 的 references 中；该入口 skill 现已归档禁用，不再作为 active skill 路径的一部分。
...
# 查看技能的引用文件
skill_view("writing-skills", "anthropic-best-practices.md")
```
